...
Kaasaegne veebirakendus ei ole üks programm. See koosneb erinevatest komponentidest: andmebaas hoiab andmeid, API server käsitleb ärilogikat, frontend näitab kasutajaliidet, cache teeb süsteemi kiireks. Igaüks neist töötab eraldi konteineris. Iga komponent on teadlikult eraldatud, et saaks seda sõltumatult arendada, testida ja skaleerida - see on mikroteenuste arhitektuuri põhiprintsiip.
...
## 4. Teenused - mis need on ja kuidas neid kirjeldada
...
## 5. Kuidas teenused üksteist leiavad
...
## 7. Teenuste järjekord ja sõltuvused
...
## 8. Keskkonnamuutujad ja konfiguratsioon
...
Kui midagi läheb valesti, vaata logisid: `docker-compose logs`. Lisades `-f` saad jälgida reaalajas (follow), lisades teenuse nime näed ainult selle teenuse logisid. `-f` flag töötab sarnaselt `tail -f` käsule UNIX'is - see jääb terminali kinni ja näitab uusi logiridu nagu nad tulevad.
...
Kui tahad kõik maha võtta ja puhastada, kasuta `down`. See kustutab konteinerid. Aga volumes jäävad alles - kui tahad ka need kustutada (ANDMED KAOVAD!), lisa `-v`. `docker-compose down -v` on ohtlik käsk - kasuta seda ainult arenduses, mitte kunagi produktsioonis ilma varunduseta.
...
Kui midagi ei tööta, on süsteemne lähenemine. Debugging võib tunduda keeruline, aga kui lähed sammsammult, leiad probleemi alati.
...
**Tavaline probleem:** Kui bind mount'id ei tööta Mac'is või Windows'is, kontrolli Docker Desktop'i settings'ist kas file sharing on lubatud sellele kaustale. Docker peab saama luba host'i failidele ligi pääseda.
